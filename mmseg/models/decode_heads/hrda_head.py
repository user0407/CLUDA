...
class HRDAHead(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        if single_scale_head == 'DAFormerHead':
            ...
            feat_cfg['channels'] = attention_embed_dim
            feat_cfg['decoder_params']['embed_dims'] = attention_embed_dim
            if feat_cfg['decoder_params']['fusion_cfg']['type'] == 'aspp':
                feat_cfg['decoder_params']['fusion_cfg'] = dict(
                    type='conv',
                    kernel_size=1,
                    act_cfg=dict(type='ReLU'),
                    norm_cfg=feat_cfg['decoder_params']['fusion_cfg']
                    ['norm_cfg'])
        ...
        feat_cfg['type'] = single_scale_head
        feat_cfg['num_classes'] = 1
        ...
        if fixed_attention is None:
            ...
            self.feat_weighter = builder.build_head(feat_cfg)
        else:
    ...
    def hr_crop_slice(self, scale):
        if isinstance(self.hr_crop_box, list):
            crop_y11, crop_y12, crop_x11, crop_x12 = scale_box(self.hr_crop_box[0],
                                                           scale)
            crop_y21, crop_y22, crop_x21, crop_x22 = scale_box(self.hr_crop_box[1],
                                                           scale) 
            return slice(crop_y11, crop_y12), slice(crop_x11, crop_x12), \
                   slice(crop_y21, crop_y22), slice(crop_x21, crop_x22)
        else:                              
    ...
    def decode_hr(self, inp, bs, return_fused=False):
        if isinstance(inp, dict) and 'boxes' in inp.keys():
            ...
            if return_fused:
                return self.head(inp, return_fused = True)
            else:
                return self.head(inp)

    def get_scale_attention(self, inp, return_fused=False):
        if self.scale_attention is not None:
            att = self.scale_attention(inp, return_fused=return_fused)
    ...
    def forward(self, inputs, return_fused=False):
        assert len(inputs) == 2
        hr_inp = inputs[1]
        ...
        if return_fused:
            flr, lr_seg = self.head(lr_inp, return_fused=True)
            flr = F.interpolate(flr,
                                scale_factor=hr_scale / lr_scale,
                                mode='bilinear',
                                align_corners=True)
        else:
            lr_seg = self.head(lr_inp)
        if return_fused:
            fused_feats_hr, hr_seg = self.decode_hr(hr_inp, batch_size,
                                                    return_fused = True)
        else:
            hr_seg = self.decode_hr(hr_inp, batch_size)
        att = self.get_scale_attention(lr_sc_att_inp, return_fused=False)
        wt = torch.sigmoid(self.feat_weighter(lr_sc_att_inp))
        if has_crop:
            mask = lr_seg.new_zeros([lr_seg.shape[0], 1, *lr_seg.shape[2:]])
            sc_os = self.os / lr_scale
            slc = self.hr_crop_slice(sc_os)
            if isinstance(self.hr_crop_box, list):
                mask[0][:, slc[0], slc[1]] = 1
                mask[1][:, slc[2], slc[3]] = 1
            else:
                mask[:, :, slc[0], slc[1]] = 1
            att = att * mask
        ...
        if torch.is_tensor(att):
            ...
            wt = self.resize(wt, hr_scale / lr_scale)
        if has_crop:
            hr_seg_inserted = torch.zeros_like(up_lr_seg)
            slc = self.hr_crop_slice(self.os)
            if isinstance(self.hr_crop_box, list):
                hr_seg_inserted[0][:, slc[0], slc[1]] = hr_seg[0]
                hr_seg_inserted[1][:, slc[2], slc[3]] = hr_seg[1]
            else:
                hr_seg_inserted[:, :, slc[0], slc[1]] = hr_seg
        else:
            hr_seg_inserted = hr_seg

        fused_seg = att * hr_seg_inserted + (1 - att) * up_lr_seg
        if return_fused:
            fhr = torch.zeros_like(flr)
            slc = self.hr_crop_slice(self.os)
            if isinstance(self.hr_crop_box, list):
                fhr[0][:, slc[0], slc[1]] = fused_feats_hr[0]
                fhr[1][:, slc[2], slc[3]] = fused_feats_hr[1]
            else:
                fhr[:, :, slc[0], slc[1]] = fused_feats_hr
        ...
        if return_fused:
            return fused_seg, lr_seg, hr_seg, flr, fhr, mask, wt
        else:
    ...
    def forward_train(self,
        ...
        seg_logits = self.forward(inputs, return_fused=True)
        losses = self.losses(seg_logits[:3], gt_semantic_seg, seg_weight)
        self.reset_crop()
        if return_fused:
            losses['fused_feats_lr'] = seg_logits[3]
            losses['fused_feats_hr'] = seg_logits[4]
            losses['mask'] = seg_logits[5]
            losses['att'] = seg_logits[6]
        return losses
    ...
    def forward_test_mix(self, inputs):
