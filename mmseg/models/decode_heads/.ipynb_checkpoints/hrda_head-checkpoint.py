...
class HRDAHead(BaseDecodeHead):
    ...
    def get_scale_attention(self, inp):
        if self.scale_attention is not None:
            att_feat, att = self.scale_attention(inp, return_fused=True)
    ...
    def forward(self, inputs, return_fused=False):
        ...
        if return_fused:
            fused_feats_lr, lr_seg = self.head(lr_inp, return_fused=True)
            fused_feats_lr = self.resize(fused_feats_lr, hr_scale / lr_scale)
        else:
            lr_seg = self.head(lr_inp)
        # print_log(f'lr_seg {lr_seg.shape}', 'mmseg')

        if return_fused:
            fused_feats_hr, hr_seg = self.decode_hr(hr_inp, batch_size, return_fused = True)
        else:
            hr_seg = self.decode_hr(hr_inp, batch_size)

        att_feat, att = self.get_scale_attention(lr_sc_att_inp)
        # calculate entropy
        ent = get_entropy(att).unsqueeze(1)
        if has_crop:
        ...
        if torch.is_tensor(att):
            ...
            ent = self.resize(ent, hr_scale / lr_scale)
        if has_crop:
        ...
        if return_fused:
            fhr = torch.zeros_like(fused_feats_lr)
            slc = self.hr_crop_slice(self.os)
            fhr[:, :, slc[0], slc[1]] = fused_feats_hr

        #     # Compute max of attention along class dims and use that as the 
        #     # weighting factor
        #     cl_att = torch.max(att, dim = 1)[0].unsqueeze(1)
        #     rect_feats = (1 - cl_att) * fused_feats_lr + cl_att * fhr
        ...
        if return_fused:
            return fused_seg, lr_seg, hr_seg, fused_feats_lr, fhr, ent
        else:
